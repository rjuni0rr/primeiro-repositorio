<x-layouts.auth-layout subtitle="{{ empty($subtitle) ? '' : $subtitle }}">
    ...
    <div class="main-card overflow-auto">
        ...
        <div class="flex gap-4 mb-4">
            ...
            <p class="bg-zinc-100 border-1 border-slate-300 rounded-md w-full p-2">Formato: <span class="text-black font-bold">{{ getFormattedTicketNumber(0, $queue->queue_prefix, $queue->queue_total_digits) }}</span></p>
        </div>

        <div class="flex gap-4 mb-4">
            <p class="bg-zinc-100 border-1 border-slate-300 rounded-md w-1/2 p-2">Estado: <span class="text-black font-bold">{{ getQueueStateText($queue->status) }}</span></p>
        ...
        @else
            <div class="flex justify-between gap-4 my-4">
                <div class="bg-gradient-to-b from-slate-200 to-slate-50 border-1 border-slate-300 rounded-xl w-full p-4 text-center text-xl">
                    Total de tickets<br><strong class="text-3xl">{{ $queue->total_tickets }}</strong>
                </div>
                <div class="bg-gradient-to-b from-slate-200 to-slate-50 border-1 border-slate-300 rounded-xl w-full p-4 text-center text-xl">
                    Dispensados<br><strong class="text-3xl">{{ $queue->total_dismissed}}</strong>
                </div>
                <div class="bg-gradient-to-b from-slate-200 to-slate-50 border-1 border-slate-300 rounded-xl w-full p-4 text-center text-xl">
                    Não atentidos<br><strong class="text-3xl">{{ $queue->total_not_attended }}</strong>
                </div>
                <div class="bg-gradient-to-b from-slate-200 to-slate-50 border-1 border-slate-300 rounded-xl w-full p-4 text-center text-xl">
                    Chamados<br><strong class="text-3xl">{{ $queue->total_called }}</strong>
                </div>
                <div class="bg-gradient-to-b from-slate-200 to-slate-50 border-1 border-slate-300 rounded-xl w-full p-4 text-center text-xl">
                    Em espera<br><strong class="text-3xl">{{ $queue->total_waiting }}</strong>
                </div>
            </div>
                ...
                <tbody>
                    ...
                    @foreach($tickets as $ticket)
                        <tr class="border-1 border-slate-300">
                            <td class="border-1 border-slate-300">{{ getFormattedTicketNumber($ticket->queue_ticket_number, $queue->queue_prefix, $queue->queue_total_digits) }}</td>
