...
class BundlesController extends Controller
{
    ...
    public function createBundleSubmit(Request $request)
    {
        ...
        // check if the queues list has less than 8 queues
        $tmp = json_decode($request->queues_list, true);
        if(count($tmp) > 8){
            return redirect()->back()->withInput()->withErrors(['queues_list' => 'O bundle deve conter no máximo 8 filas.']);
        }
        ...
        if($bundleExists) {
            return redirect()->back()->withInput()->withErrors(['bundle_name' => 'Já existe outro bundle com o mesmo nome.']);
        }
        ...
        if(count($valid_queues) !== count($queues_hash_codes)) {
            return redirect()->back()->withInput()->withErrors(['queues_list' => 'Algumas filas selecionadas não existem ou não pertencem à sua empresa.']);
        }
    ...
    public function generateCredentialValue($num_chars)
    {
        ...
        if(!is_numeric($num_chars) || $num_chars <=0 || $num_chars > 64) {
            return response()->json(['error' => 'Invalid numver of characters'], 400);
        }
    ...
    public function restore($id)
    {
        ...
        // find the bundle by id and check if it belongs to the user's company
